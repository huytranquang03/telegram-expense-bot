...
const { getSession, saveData, getActiveMembers, getActiveExpenses } = require('../services/sessionService');
const { formatMoney, formatName, parseMoney, parseNameList, parseCustomSplit } = require('../utils/formatters');
...
bot.on('message', async (msg) => {
  ...
  if (session.activeMode === 'detailed') {
    ...
    if (billMatch) {
      ...
      const members = getActiveMembers(session);
      const existingPayer = [...members].find((m) => m.toLowerCase() === payerCandidate.toLowerCase());
      if (!existingPayer) {
        members.add(payerCandidate);
        saveData();
      }
      const actualPayer = [...members].find((m) => m.toLowerCase() === payerCandidate.toLowerCase());
    ...
    if (session.pendingBill && text.toLowerCase().startsWith('món ')) {
      ...
      const dishMatch = dishText.match(/^(.+?)\s+([0-9.,]+[kK]?)(?:\s*[:\s]\s*(.*))?$/i);

      if (!dishMatch) {
        return bot.sendMessage(chatId, '⚠️ Cú pháp món: món <tên> <số tiền>[: người1 người2]');
      }
      ...
      if (isNaN(amount) || amount <= 0) {
        return bot.sendMessage(chatId, '⚠️ Số tiền món không hợp lệ.');
      }
      ...
      const participantText = participants.length > 0 ? participants.join(', ') : 'Cả nhóm';
      return bot.sendMessage(
        chatId,
    ...
    if (session.pendingBill && text.toLowerCase() === 'xong') {
      ...
      if (items.length === 0) {
        ...
        saveData();
        return bot.sendMessage(chatId, '⚠️ Chưa có món nào. Đã hủy bill.');
      }
      ...
      items.forEach((item) => {
        const itemParticipants = item.participants.length > 0 ? item.participants : [...members];
        expenses.push({
          payer,
          amount: item.amount,
      ...
      const warning = totalFromItems !== totalPaid
    ...
    if (session.pendingBill && text.toLowerCase() === 'hủy') {
      session.pendingBill = null;
      saveData();
      return bot.sendMessage(chatId, '❌ Đã hủy bill.');
    }
  }
  ...
  if (text.startsWith('+')) {
    ...
    if (names.length > 0) {
      names.forEach((n) => {
        ...
        if (!exists) members.add(n);
      });
      saveData();
      return bot.sendMessage(chatId, `✅ Đã thêm: *${names.join(', ')}*`, { parse_mode: 'Markdown' });
    }
    return;
  }
  ...
  const expenseMatch = textWithoutChia.match(/^(.*?)\s+([0-9.,]+[kK]?)(?:\s+(.*))?$/i);
  if (!expenseMatch) return;

  const payerCandidate = formatName(expenseMatch[1]);
  const amountStr = expenseMatch[2];
  let description = expenseMatch[3] ? expenseMatch[3].trim() : 'Đóng góp quỹ chung';
  ...
  // Check for custom split amounts (Name:Amount format)
  ...
  if (!customAmounts && participants.length < 1) {
    return bot.sendMessage(chatId, '⚠️ Không tìm thấy ai để chia tiền.');
  }

  const expenses = getActiveExpenses(session);
  expenses.push({ payer: actualPayer, amount, description, participants, isSplitAll, customAmounts });
  saveData();
  ...
  if (customAmounts && customAmounts.length > 0) {
    ...
    const actualCount = isSplitAll ? members.size : participants.length;
